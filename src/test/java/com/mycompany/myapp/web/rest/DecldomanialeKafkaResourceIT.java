...
class DecldomanialeKafkaResourceIT {
    ...
    void setup() {
        ...
        DecldomanialeKafkaResource kafkaResource = new DecldomanialeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/decldomaniale-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/decldomaniale-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
